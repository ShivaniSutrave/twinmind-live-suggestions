/* =============================================
   ...
   Features: Live transcript, Smart Notes, History,
   New Topic, Short answers, No bad words
   ============================================= */
...
function loadSettings() {
  state.apiKey = localStorage.getItem('tm_apikey') || '';
  try {
...
function updateLiveTranscript(interim, final) {
  ...
  if (final.trim()) {
    ...
    // Detect meeting type after enough words
    if (state.fullTranscript.split(' ').length === 25 && state.meetingType === 'General') {
      detectMeetingType();
    }
  }
...
// ───────── NEW TOPIC ─────────
function newTopic() {
  if (!state.isRecording && state.fullTranscript.trim().length < 10) return;

  state.topicCount++;
  state.recentTranscript = '';
  state.meetingType = 'General';

  // Visual divider in transcript
  const box = document.getElementById('transcript-box');
  const div = document.createElement('div');
  ...
  // Clear suggestion batches visually for fresh start
  const container = document.getElementById('suggestions-container');
  container.querySelectorAll('.batch-group').forEach(g => {
    g.style.opacity = '0.2';
...
async function detectMeetingType() {
  ...
  try {
    const res = await callGroq([{
      role: 'user',
      content: `What type of meeting is this? Reply with ONLY 2-3 words. Examples: Job Interview, Sales Call, Technical Meeting, Team Standup, Brainstorm, Award Speech, Client Meeting, Lecture.\n\nTranscript: "${state.fullTranscript.slice(0, 300)}"`
    }], 15);
...
function scheduleAutoRefresh() {
  ...
  state.autoRefreshTimeout = setTimeout(async () => {
    if (state.isRecording && state.recentTranscript.trim().split(' ').length > 5) {
...
async function manualRefresh() {
  if (!state.apiKey) { openSettings(); return; }
  if (!state.recentTranscript.trim() && !state.fullTranscript.trim()) {
    alert('Start recording and speak first!'); return;
  }
  clearTimeout(state.autoRefreshTimeout);
  await generateSuggestions();
  if (state.isRecording) scheduleAutoRefresh();
...
async function generateSuggestions() {
  ...
  try {
    // Use only RECENT transcript for suggestions (last N words)
    ...
    const raw = await callGroq([{
      role: 'user',
      content: `Recent conversation (last ~${words} words only):\n"${recentText}"\n\n${prompt}`
    }], 500);
    ...
    btn.disabled = false;
    btn.innerHTML = '<span class="reload-icon">↻</span> Reload suggestions';
  }
}
...
function addSuggestionBatch(suggestions) {
  ...
  container.querySelectorAll('.batch-group:not(.faded-topic)').forEach(g => g.classList.add('old'));
...
function createSuggestionCard(suggestion, batchId, index) {
  ...
  card.addEventListener('click', (e) => {
    ...
    openSuggestionInChat(suggestion);
  });
  return card;
}
...
function scheduleNotesRefresh() {
  // Generate notes every 35 seconds
  state.notesRefreshTimeout = setTimeout(async () => {
    if (state.isRecording && state.recentTranscript.trim().split(' ').length > 8) {
      await generateNotes();
    }
  ...
  try {
    ...
    notes.forEach(note => {
      if (note.text && note.text.trim()) {
        addNoteCard(note);
        state.notes.push({ ...note, ts: new Date().toISOString() });
      }
    });
...
async function sendChatMessage() {
  ...
  if (!question || !state.apiKey) { if (!state.apiKey) openSettings(); return; }
  input.value = '';
...
async function getDetailedAnswer(question) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      contentEl.textContent = 'Error: ' + (err.error?.message || 'API error');
      return;
    }
    ...
    while (true) {
      ...
      for (const line of decoder.decode(value).split('\n')) {
        ...
        try {
          ...
          fullAnswer += delta;
          contentEl.textContent = sanitizeText(fullAnswer);
    ...
    contentEl.classList.remove('streaming');
    contentEl.textContent = 'Network error. Check your API key.';
    console.error(err);
  }
}
...
async function showPostMeetingSummary() {
  ...
  try {
    const prompt = (state.settings.summaryPrompt || DEFAULTS.summaryPrompt)
      .replace('{{TRANSCRIPT}}', state.fullTranscript);
    ...
    const renderList = (arr) => (arr||[]).length
      ...
      : '<p style="color:var(--text-muted)">None noted</p>';
    ...
    content.innerHTML = '<p style="color:var(--text-muted)">Could not generate summary.</p>';
    console.error(err);
  }
}
...
function saveSessionToHistory() {
  const session = {
    ...
    preview: state.fullTranscript.slice(0, 150),
  };
...
function loadSessionHistory() {
  // Already in localStorage, just loaded on demand
}
...
function openHistory() {
  ...
  if (history.length === 0) {
    content.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:20px">No past meetings saved yet. Complete a recording session to save it here.</p>';
  } else {
...
function loadHistorySession(id) {
  ...
  const transcript = (session.transcriptChunks || [])
    .map(c => `<div class="transcript-chunk"><span class="ts">${c.ts}</span>${escapeHtml(c.text)}</div>`)
    .join('');

  const suggestions = (session.suggestionBatches || []).map((b, i) => `
    <div style="margin-bottom:12px">
      <div class="batch-label">BATCH ${i+1} · ${b.ts}</div>
      ${(b.suggestions||[]).map(s => `
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin-bottom:6px">
          <span class="card-tag tag-${(s.type||'ANSWER').replace(/\s+/g,'_')}">${escapeHtml((s.type||'').replace(/_/g,' '))}</span>
          <div style="font-size:0.82rem;margin-top:5px;color:var(--text)">${escapeHtml(s.preview||'')}</div>
        </div>`).join('')}
    </div>`).join('');

  const chat = (session.chatHistory || []).map(m => `
    <div class="chat-bubble ${m.role}" style="margin-bottom:6px">
      <div class="bubble-label">${m.role === 'user' ? 'YOU' : 'ASSISTANT'}</div>
  ...
  content.innerHTML = `
    ...
    <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:16px">
      ...
      <div style="font-size:0.72rem;font-family:var(--font-mono);color:var(--text-muted)">${date} · Duration: ${duration}</div>
    </div>

    <div style="margin-bottom:16px">
      <div style="font-size:0.72rem;font-family:var(--font-mono);color:var(--accent2);margin-bottom:8px;letter-spacing:0.08em">TRANSCRIPT</div>
      <div style="max-height:180px;overflow-y:auto;font-size:0.82rem;line-height:1.6">${transcript || '<p style="color:var(--text-muted)">No transcript</p>'}</div>
    </div>

    <div style="margin-bottom:16px">
      <div style="font-size:0.72rem;font-family:var(--font-mono);color:var(--accent2);margin-bottom:8px;letter-spacing:0.08em">SUGGESTIONS</div>
      <div style="max-height:200px;overflow-y:auto">${suggestions || '<p style="color:var(--text-muted)">No suggestions</p>'}</div>
    </div>

    <div style="margin-bottom:16px">
      <div style="font-size:0.72rem;font-family:var(--font-mono);color:var(--accent2);margin-bottom:8px;letter-spacing:0.08em">CHAT HISTORY</div>
      <div style="max-height:180px;overflow-y:auto">${chat || '<p style="color:var(--text-muted)">No chat history</p>'}</div>
    </div>
...
function downloadSession(id) {
  ...
  a.href = url;
  a.download = `meeting-${session.meetingType.replace(/\s+/g,'-')}-${session.id}.json`;
  a.click();
...
function startTimer() {
  ...
  state.timerInterval = setInterval(() => {
    state.timerSeconds++;
    el.textContent = formatDuration(state.timerSeconds);
  }, 1000);
}
...
function updateMicUI(recording) {
  ...
  if (recording) {
    ...
    document.getElementById('mic-label').textContent = 'Click to start';
...
function sanitizeText(text) {
  // Remove profanity / bad words by replacing with asterisks
  const badWords = ['fuck','shit','ass','bitch','damn','hell','crap','bastard','idiot','stupid'];
...
function getSupportedMimeType() {
  return ['audio/webm;codecs=opus','audio/webm','audio/ogg','audio/mp4'].find(t=>MediaRecorder.isTypeSupported(t))||'audio/webm';
}
